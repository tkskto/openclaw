...
function isEquivalentVisibleReplyText(a?: string, b?: string): boolean {
  if (!a || !b) {
    return false;
  }
  if (a === b) {
  ...
  const normalizedA = normalizeTextForComparison(a);
  const normalizedB = normalizeTextForComparison(b);
...
export function handleMessageEnd(
  ...
  if (
    ...
    } else if (!isEquivalentVisibleReplyText(text, ctx.state.lastBlockReplyText) || hasMedia) {
